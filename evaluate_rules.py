...
for _, row in df.iterrows():
    ...
    if rid in llm_cache:
        records.append({
            ...
            "llm_subcategory": llm_cache[rid]["subcategory"],
            "llm_explanation": llm_cache[rid]["explanation_de"]
        })

print(f"Loaded {len(records)} reports aligned with LLM classifications.")
...
def classify_regex(text, category_id):
    ...
    if category_id == 7:
        ...
        # Check if there are strong unrelated words
        # but allow if it explicitly mentions a cycle path or bicycle
        allowed_overrides = [
        ...
        # If it's in a cycling relevant category and does not have strong unrelated indicators
        # Ignore negation if it is a major infrastructure hazard like ampel or sign obstruction
